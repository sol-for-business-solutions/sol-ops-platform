...
const SEV: Record<string, { dot: string; bg: string; text: string; label: string }> = {
  info:      { dot: '#142680', bg: '#eff6ff', text: '#1d4ed8', label: 'Info' },
  warning:   { dot: '#d97706', bg: '#fffbeb', text: '#92400e', label: 'Warning' },
  critical:  { dot: '#ea580c', bg: '#fff7ed', text: '#9a3412', label: 'Critical' },
  emergency: { dot: '#dc2626', bg: '#fef2f2', text: '#991b1b', label: 'Emergency' },
}
...
export function RecentFlags() {
  ...
  if (flags.length === 0) return (
    <div className="flex items-center justify-center py-8 gap-3">
      ...
      <p className="text-sm text-gray-400">No open flags — all clear</p>
    </div>
  )
  return (
          ...
          <Link key={flag.id} href={`/dashboard/flags?course=${flag.course_id}`}
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold px-2 py-0.5 rounded-md" style={{background:s.bg,color:s.text}}>{s.label}</span>
