  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
        <p className="text-sm font-medium text-gray-700">{presentCount} / {trainees.length} {t('attendance.present')}</p>
        {canEdit && <button onClick={onMarkAll} className="text-xs text-gray-500 hover:text-gray-900 underline transition-colors">{t('attendance.markAll')}</button>}
      </div>
            ...
            <div key={trainee.id} className={`flex items-center gap-4 px-4 py-3 transition-colors ${present ? 'bg-green-50/50' : ''}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="text-sm font-medium text-gray-900 truncate">{trainee.full_name_en}</p>
                  {eligible && <span title={t('attendance.eligible')}><Award size={13} className="text-amber-500 shrink-0" /></span>}
                </div>
              ...
              <div className="shrink-0 text-right">
                ...
                <p className="text-xs text-gray-400 mt-0.5">{count}/4 {t('attendance.sessions')}</p>
              </div>
            </div>
          )
        })}
