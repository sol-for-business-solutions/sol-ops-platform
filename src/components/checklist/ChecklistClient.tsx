...
export function ChecklistClient({ courses, initialCourseId, role }: Props) {
  ...
  const phaseLabels = { pre: 'Pre-course', during: 'During course', post: 'Post-course' }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-xl font-semibold text-gray-900">Checklists</h1><p className="text-sm text-gray-500 mt-0.5">Track pre, during, and post-course tasks</p></div>
      </div>
      {courses.length === 0 ? <EmptyState title="No active courses" description="Checklists appear here for scheduled and in-progress courses" /> : (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3">
            {!selectedCourseId ? <EmptyState title="Select a course to view its checklist" /> : loading ? <div className="flex justify-center py-16"><Spinner /></div> : (
              <>
