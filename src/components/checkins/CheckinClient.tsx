...
export function CheckinClient({ courses, initialCourseId, profile, role }: Props) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6"><div><h1 className="text-xl font-semibold text-gray-900">Check-in / Check-out</h1><p className="text-sm text-gray-500 mt-0.5">GPS-verified presence at course venue</p></div></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div>
            ...
            <div className="space-y-1.5">
              {courses.map(course => (
                <button key={course.id} onClick={() => { setSelectedCourseId(course.id); setLastResult(null) }}
          ...
          <div className="lg:col-span-2 space-y-4">
            {!selectedCourse ? <EmptyState title="Select a course to check in" /> : loading ? <div className="flex justify-center py-16"><Spinner /></div> : (
              <>
