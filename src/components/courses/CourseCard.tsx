...
interface Props {
  course: Course & { city?: { name_en: string; name_ar: string }; course_assignments?: { coordinator: { full_name: string } }[] }
...
export function CourseCard({ course, role, onUpdate }: Props) {
  ...
  const cityName = locale === 'ar' ? (course.city as any)?.name_ar : course.city?.name_en
      ...
      <Link href={`/dashboard/courses/${course.id}`}
        ...
        <div className="p-5">
          <div className="flex items-start justify-between mb-3">
            <Badge label={t(`status.${course.status}`) ?? course.status} variant={course.status} />
            <ArrowUpRight size={16} className="text-gray-300 group-hover:text-[#142680] transition-colors mt-0.5" />
          </div>
          <h3 className="font-bold text-gray-900 mb-0.5 line-clamp-1 group-hover:text-[#142680] transition-colors">
            {locale === 'ar' ? course.title_ar : course.title_en}
          </h3>
          <p className="text-sm text-gray-400 mb-4 line-clamp-1" dir={locale === 'ar' ? 'ltr' : 'rtl'}>
            {locale === 'ar' ? course.title_en : course.title_ar}
          </p>
          <div className="space-y-2 pt-3" style={{borderTop:'1px solid #f0f4ff'}}>
            <div className="flex items-center gap-2 text-sm text-gray-500">
              ...
              <span className="truncate">{cityName ?? '—'} · {course.venue}</span>
            </div>
            <div className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-500">
              ...
              <span className="truncate">{coordinators.length > 0 ? coordinators.map(a => a.coordinator?.full_name).join(', ') : t('courses.noCoordinator')}</span>
            </div>
          </div>
        </div>
      </Link>
      {canClone && (
        <button onClick={handleClone} title={t('courses.cloneTitle')}
