interface City { id: string; name_en: string; region_en: string; total: number; active: number; scheduled: number; completed: number; openFlags: number; trainees: number }
export function CityTable({ cities }: { cities: City[] }) {
  const active = cities.filter(c => c.total > 0)
  if (active.length === 0) return <p className="text-sm text-gray-400 text-center py-8">No city data yet</p>
  return (
    <div className="overflow-x-auto -mx-1">
      <table className="w-full text-sm">
        <thead>
          <tr style={{borderBottom:'1px solid #e8edf5'}}>
            {['City','Active','Scheduled','Completed','Flags','Trainees'].map(h => (
              <th key={h} className={`py-2 px-3 text-xs font-bold text-gray-400 uppercase tracking-wide ${h==='City'?'text-left':'text-right'}`}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {active.map(city => (
            <tr key={city.id} className="hover:bg-blue-50/50 transition-colors" style={{borderBottom:'1px solid #f0f4ff'}}>
              <td className="py-3 px-3">
                <p className="font-semibold text-gray-800">{city.name_en}</p>
                <p className="text-xs text-gray-400">{city.region_en}</p>
              </td>
